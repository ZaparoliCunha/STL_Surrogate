# Random Forest based surrogate of the Sound Transmission Loss analyses with 
# Sensitivity analysis evaluation based on Mean decrease in impurity (MDI).
...
def regressor_pipe():
    ...
    #Evaluate feature importance and thier standard deviation
    importances = []; std = [];
    if Evaluate_Feat_Imp == True:
        for i in range(y_train.shape[1]):
            importances.append(list(Pipe_Final.regressor_.named_steps['regressor'].estimators_[i].feature_importances_))    
            std.append(list(np.std([tree.feature_importances_ for tree in Pipe_Final.regressor_.named_steps['regressor'].estimators_[i].estimators_], axis=0)))
            writer_feat_import.writerow(importances[i])
            writer_feat_import_std.writerow(std[i])
...
if physics_features == True:        
    ...
    features_latex.append('$D_R$')
if resonant_feature ==  True: features_latex.append('$\\frac{D}{m(a^4 b^4)}$') 


# %% Define which analysis to run
Cross_val = False     # Run Cross validation with n_kfold
n_kfold = 5
Evaluate_Feat_Imp = True # Perform Sensitivity Analysis
...
if Evaluate_Feat_Imp == True:    
    file_feat_import = open('Results/'+'FI_'+pre_name+'.csv','w')
    writer_feat_import = csv.writer(file_feat_import)
    file_feat_import_std = open('Results/'+'FI_'+'std_'+pre_name+'.csv','w')
    writer_feat_import_std = csv.writer(file_feat_import_std)
...
# Number of supporting points: "N_samples = 0" use the entire dataset
N_samples = 0

## Define File Name and create folder 
...
std_y = output_transf  #output transformation
inp_sh = len(inpt_var) #number of input features
out_sh = Y.shape[1]    #number of output values

# Print information and write report
n_samples = X.shape[0]        
...
regressor = MultiOutputRegressor(RandomForestRegressor(n_estimators = n_estimators, 
                                                        random_state = seed,criterion = 'squared_error',
                                                        max_depth = max_depth, max_features = max_features,
                                                        max_samples = max_samples,n_jobs = -1),n_jobs = -1)
regressor_pipe()
...
file_report.write("#################################################################### \n")
file_report.close()
if Evaluate_Feat_Imp == True: file_feat_import.close()
